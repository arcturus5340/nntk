    ...
    public
      ...
      begin
        ...
        for var index := 1 to self.number_of_layers-1 do
          self.neural_network[index-1] := new Layer(trunc(neural_network_topology[0, index]),
                                                  trunc(neural_network_topology[0, index-1]));
        end;
      ...
      begin
      if input_data.shape()[0] <> self.topology.shape()[0] then
      ...
      begin
        result := new neo.neo_array(1, size);
        {$omp parallel for}
        for var index := 0 to size-1 do
          result[0, index] := random() < global_dropout_probability? 0: 1;
      end;
  end;
...
//            else if self.creation_op = "transpose" then 
//              self.creators[0].backward(self.grad.transpose(), self)
              ...
              begin
                ...
                var ds := self.creators[0].data.shape[dim];
//                self.creators[0].backward(self.grad.expand(dim,ds));
              end
            else if self.creation_op.StartsWith('expand') then 
              begin
//              var dim := integer.Parse(self.creation_op.Split('_')[1]);
...
//        end;
      ...
      begin
        ...
        for var index := 0 to self.data.shape()[0] do
          result += self.data[0, index] + ', ';
        result += self.data[0, self.data.shape()[0]-1].ToString + ' (';
        ...
        if self.grad <> nil then
          for var index := 0 to self.grad.data.shape()[0]-1 do
